...
import { BrotliBuildHuffmanTable, HuffmanCode } from "./huffman.js";
...
export function BrotliDecompressedSize(buffer) {
...
export function BrotliDecompressBuffer(buffer, output_size) {
  var input = new BrotliInput(buffer);
...
export function BrotliDecompress(input, output) {
  var i;
  var pos = 0;
  var input_end = 0;
  var window_bits = 0;
  ...
  while (!input_end) {
    ...
    num_direct_distance_codes =
      NUM_DISTANCE_SHORT_CODES + (br.readBits(4) << distance_postfix_bits);

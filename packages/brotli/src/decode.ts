...
const MAX_HUFFMAN_TABLE_SIZE: Int32Array = Int32Array.from([
  256, 402, 436, 468, 500, 534, 566, 598, 630, 662, 694, 726, 758, 790, 822, 854, 886, 920, 952,
  984, 1016, 1048, 1080,
]);
const CODE_LENGTH_CODE_ORDER: Int32Array = Int32Array.from([
  1, 2, 3, 4, 0, 5, 17, 6, 16, 7, 8, 9, 10, 11, 12, 13, 14, 15,
]);
const DISTANCE_SHORT_CODE_INDEX_OFFSET: Int32Array = Int32Array.from([
  0, 3, 2, 1, 0, 0, 0, 0, 0, 0, 3, 3, 3, 3, 3, 3,
]);
const DISTANCE_SHORT_CODE_VALUE_OFFSET: Int32Array = Int32Array.from([
  0, 0, 0, 0, -1, 1, -2, 2, -3, 3, -1, 1, -2, 2, -3, 3,
]);
const FIXED_TABLE: Int32Array = Int32Array.from([
...
function calculateDistanceAlphabetSize(
  npostfix: number,
  ndirect: number,
  maxndistbits: number,
): number {
...
function calculateDistanceAlphabetLimit(
  s: State,
  maxDistance: number,
  npostfix: number,
  ndirect: number,
...
function attachDictionaryChunk(s: State, data: Int8Array): number {
  ...
  if (s.cdNumChunks === 0) {
    s.cdChunks = Array.from<Int8Array>({ length: 16 });
...
function readComplexHuffmanCode(
  ...
  const result: number = readHuffmanCodeLengths(
...
function readMetablockPartition(s: State, treeType: number, numBlockTypes: number): number {
  ...
  let result: number = readHuffmanCode(
...
function readMetablockHuffmanCodesAndContextMaps(s: State): number {
  ...
  let distanceAlphabetSizeMax: number = calculateDistanceAlphabetSize(
  ...
  s.distanceTreeGroup = new Int32Array(
    huffmanTreeGroupAllocSize(distanceAlphabetSizeLimit, numDistTrees),
  );
...
function doUseDictionary(s: State, fence: number): number {
  ...
  if (address < 0) {
    ...
    const len: number = transformDictionaryWord(
      s.ringBuffer,
      s.pos,
      dictionaryData,
      offset,
...
function unpackTransforms(
  prefixSuffix: Int8Array,
  prefixSuffixHeads: Int32Array,
  transforms: Int32Array,
...
let data: ByteBuffer = new Int8Array(0);
...
class InputStream {
  data: ByteBuffer = new Int8Array(0);
  offset = 0;
...
type ByteBuffer = Int8Array | Uint8Array<ArrayBufferLike>;

/**
 * Decodes brotli stream.
 */
export function brotliDecode(bytes: ByteBuffer, options?: BrotliDecodeOptions): Int8Array {
  const s = new State();

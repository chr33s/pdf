...
import { readdir, readFile } from "./utilts.js";

const brotliBinary = await readFile("../dist/decode.js");
...
describe("decompress", function () {
  test.each(compressedFiles)(`%s`, async function (file) {
    ...
    const result = decode(new Int8Array(compressed));
  ...
  test("should match node:zlib#brotli", function () {
    ...
    const result = decode(new Int8Array(data));

...
describe("variations", function () {
  describe.runIf(hasSkiaFont)("Skia", function () {
    ...
    test("recomputes cbox and advance width", function () {
      ...
      expect(Math.round(glyph.advanceWidth * 100) / 100).toBe(1540);
      return expect(Math.round(glyph.cbox.minX * 100) / 100).toBe(179);
    });
  });
  ...
  describe("CFF2 variations", function () {
    ...
    test("applies variations to CFF2 glyphs", function () {
      ...
      expect(font.getVariation({ wght: 500 }).glyphsForString("$")[0].path.toSVG()).toBe(

...
describe("variations", function () {
  describe.runIf(hasSkiaFont)("Skia", function () {
    ...
    test("should get available variation axes", function () {
      ...
      expect(Math.round(axes.wght.min * 100) / 100).toBe(0.48);
      expect(axes.wght.default).toBe(1);
      return expect(Math.round(axes.wght.max * 100) / 100).toBe(3.2);
    });
    ...
    test("should get named variation instances", function () {
      ...
      expect(Math.round(named.Bold.wght * 100) / 100).toBe(1.95);
      return expect(named.Bold.wdth).toBe(1);
    });

    test("should get a variation by name", async function () {
      let variation = font.getVariation("Bold");
      expect(variation.constructor.name).toBe("TTFFont");

      let glyph = variation.getGlyph(68); // D
      return expect((await glyph.getPath()).toSVG()).toBe(
    ...
    test("should get a variation by settings", async function () {
  ...
  describe("truetype variations", function () {
    test("should support sharing all points", async function () {
      ...
      expect(
        (await font.getVariation({ wght: 300 }).glyphsForString("彌")[0].getPath()).toSVG(),
      ).toBe(
    ...
    test("should use the HVAR table when available for variation metrics", async function () {
      ...
      expect(
        Math.round(
          await font.getVariation({ wght: 150 }).glyphsForString("O")[0].getAdvanceWidth(),
        ),
      ).toBe(706);
    });
    ...
    test("should fall back to the last entry in an HVAR table", async function () {
      ...
      expect(
        Math.round(
          await font.getVariation({ wght: 400 }).glyphsForString("A")[0].getAdvanceWidth(),
        ),
      ).toBe(584);
    });
  ...
  describe("CFF2 variations", function () {
    ...
    test("applies variations to CFF2 glyphs", async function () {
      expect(
        (await font.getVariation({ wght: 100 }).glyphsForString("$")[0].getPath()).toSVG(),
      ).toBe(
      ...
      expect(
        (await font.getVariation({ wght: 500 }).glyphsForString("$")[0].getPath()).toSVG(),
      ).toBe(

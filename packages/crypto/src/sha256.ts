...
(function () {
  ...
  while (nPrime < 64) {
    if (isPrime(n)) {
      if (nPrime < 8) {
        H_INIT[nPrime] = getFractionalBits(Math.pow(n, 1 / 2));
      }
...
export class SHA256 extends Hasher {
  ...
  protected _getInitialHash(): number[] {
    return H_INIT.slice(0);
  }

  protected _hash: WordArray = new WordArray(this._getInitialHash());
  ...
  protected _doReset(): void {
    this._hash = new WordArray(this._getInitialHash());
  }
  ...
  override clone(): SHA256 {
    const clone = super.clone() as SHA256;

/**
 * Rabbit stream cipher algorithm (legacy version).
 *
...
export class RabbitLegacy extends StreamCipher {
  static override keySize = 128 / 32;
  static override ivSize = 64 / 32;
  ...
  protected override _doReset(): void {
    ...
    // Generate initial state values (no endian swap in legacy version)
    const X = (this.#X = [
      K[0],
      (K[3] << 16) | (K[2] >>> 16),
      K[1],
  ...
  protected _doProcessBlock(M: number[], offset: number): void {
    ...
    for (let i = 0; i < 4; i++) {
      // Swap endian
      S[i] =
        (((S[i] << 8) | (S[i] >>> 24)) & 0x00ff00ff) | (((S[i] << 24) | (S[i] >>> 8)) & 0xff00ff00);
 ...
 * @example
 *
 *     const ciphertext = RabbitLegacyHelper.encrypt(message, key, cfg);
 *     const plaintext  = RabbitLegacyHelper.decrypt(ciphertext, key, cfg);
 */
export const RabbitLegacyHelper = createCipherHelper(RabbitLegacy);

...
export class SHA512 extends Hasher {
  ...
  protected _getInitialHash(): X64WordArray {
    return DEFAULT_HASH();
  }

  protected _hash: X64WordArray = this._getInitialHash();

  override blockSize = 1024 / 32;
  ...
  protected _doReset(): void {
    this._hash = this._getInitialHash();
  }
  ...
  override clone(): SHA512 {
    const clone = super.clone() as SHA512;
 ...
 */
export function sha512(message: WordArray | string): WordArray {
 ...
 */
export const HmacSHA512 = createHmacHelper(SHA512);

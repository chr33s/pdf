/**
 * DES and Triple-DES block cipher algorithms.
 */
...
export class DES extends BlockCipher {
  static override keySize = 64 / 32;
  static override ivSize = 64 / 32;
  override blockSize = 64 / 32;
  ...
  #exchangeLR(offset: number, mask: number): void {
    const t = ((this.#lBlock >>> offset) ^ this.#rBlock) & mask;
    this.#rBlock ^= t;
    this.#lBlock ^= t << offset;
  }

  #exchangeRL(offset: number, mask: number): void {
 ...
 * @example
 *
 *     const ciphertext = DESHelper.encrypt(message, key, cfg);
 *     const plaintext  = DESHelper.decrypt(ciphertext, key, cfg);
 */
export const DESHelper = createCipherHelper(DES);

/**
 * Triple-DES block cipher algorithm.
 */
export class TripleDES extends BlockCipher {
  static override keySize = 192 / 32;
  static override ivSize = 64 / 32;
  override blockSize = 64 / 32;
  ...
  protected override _doReset(): void {
    ...
    this.#des1 = DES.createEncryptor(new WordArray(key1)) as DES;
    this.#des2 = DES.createEncryptor(new WordArray(key2)) as DES;
    this.#des3 = DES.createEncryptor(new WordArray(key3)) as DES;
  }
 ...
 * @example
 *
 *     const ciphertext = TripleDESHelper.encrypt(message, key, cfg);
 *     const plaintext  = TripleDESHelper.decrypt(ciphertext, key, cfg);
 */
export const TripleDESHelper = createCipherHelper(TripleDES);

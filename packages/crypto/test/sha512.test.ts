import { describe, expect, test } from "vitest";
import { Hex, HMAC, HmacSHA512, SHA512, sha512, WordArray } from "../src/index.js";

describe("SHA512", () => {
  describe("hash vectors", () => {
    test("should hash empty string", () => {
      expect(sha512("").toString()).toBe(
        "cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e",
      );
    });

    test("should hash quick brown fox", () => {
      expect(sha512("The quick brown fox jumps over the lazy dog").toString()).toBe(
        "07e547d9586f6a73f73fbac0435ed76951218fb7d0c8d788a309d785436bbb642e93a252a954f23912547d1e8a3b5ed6e1bfd7097821233fa0538f3db854fee6",
      );
    });

    test("should hash quick brown fox with period", () => {
      expect(sha512("The quick brown fox jumps over the lazy dog.").toString()).toBe(
        "91ea1245f20d46ae9a037a989f54f1f790f0a47607eeb8a14d12890cea77a1bbc6c7ed9cf205e67b7f2b8fd4c7dfd3a7a8617e45f3c463d481c7e586c39ac1ed",
      );
    });
  });

  describe("update and long message", () => {
    test("should handle progressive updates", () => {
      const hasher = new SHA512();
      for (let i = 0; i < 100; i++) {
        hasher.update("12345678901234567890123456789012345678901234567890");
      }

      expect(hasher.finalize().toString()).toBe(
        "9bc64f37c54606dff234b6607e06683c7ba248558d0ec74a11525d9f59e0be566489cc9413c00ca5e9db705fc52ba71214bcf118f65072fe284af8f8cf9500af",
      );
    });
  });

  describe("clone", () => {
    test("should clone correctly", () => {
      const hasher = new SHA512();

      expect(hasher.update("a").clone().finalize().toString()).toBe(sha512("a").toString());
      expect(hasher.update("b").clone().finalize().toString()).toBe(sha512("ab").toString());
      expect(hasher.update("c").clone().finalize().toString()).toBe(sha512("abc").toString());
    });
  });

  describe("input integrity", () => {
    test("should not modify input", () => {
      const message = new WordArray([0x12345678]);
      const expected = message.toString();

      sha512(message);

      expect(message.toString()).toBe(expected);
    });
  });

  describe("helper", () => {
    test("should produce same result as class", () => {
      expect(new SHA512().finalize("").toString()).toBe(sha512("").toString());
    });
  });
});

describe("HmacSHA512", () => {
  test("should calculate HMAC correctly", () => {
    expect(
      new HMAC(SHA512, Hex.parse("0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b"))
        .finalize("Hi There")
        .toString(),
    ).toBe(HmacSHA512("Hi There", Hex.parse("0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b")).toString());
  });
});

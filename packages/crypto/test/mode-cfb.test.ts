...
import { AES, AESHelper, CFB, NoPadding, WordArray } from "../src/index.js";
...
describe("CFB Mode", () => {
  ...
  describe("encryptor", () => {
    test("should encrypt correctly with CFB mode", () => {
      // Compute expected
      const expected = message.clone();
      const aes = AES.createEncryptor(key);

      // First block XORed with encrypted IV
      const keystream1 = iv.words.slice(0);
      aes.encryptBlock(keystream1, 0);
      for (let i = 0; i < 4; i++) {
        expected.words[i] ^= keystream1[i];
      }

      // Subsequent blocks XORed with encrypted previous ciphertext block
      const keystream2 = expected.words.slice(0, 4);
      aes.encryptBlock(keystream2, 0);
      for (let i = 4; i < 8; i++) {
        expected.words[i] ^= keystream2[i % 4];
      }
  ...
  describe("decryptor", () => {
    test("should decrypt correctly with CFB mode", () => {

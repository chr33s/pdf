...
The latest release of `pdf` (`v2.0.0`) introduced a breaking change in the API for drawSVG, making it asynchronous. You'll also need to manually embed the images that might be contained inside your svg file.
...
The latest release of `pdf` (`v1.0.0`) includes several breaking API changes. If you have code written for older versions of `pdf` (`v0.x.x`), you can use the following instructions to help migrate your code to v1.0.0.
...
- To draw content on a page in old versions of `pdf`, you needed to create a content stream, invoke some operators, register the content stream, and add it to the document. Something like the following:
  ...
  However, in new versions of `pdf`, this is much simpler. You simply invoke drawing methods on the page, such as [`PDFPage.drawText`](https://pdf.js.org/docs/api/classes/pdfpage#drawtext), [`PDFPage.drawImage`](https://pdf.js.org/docs/api/classes/pdfpage#drawimage), [`PDFPage.drawRectangle`](https://pdf.js.org/docs/api/classes/pdfpage#drawrectangle), or [`PDFPage.drawSvgPath`](https://pdf.js.org/docs/api/classes/pdfpage#drawsvgpath). So the above example becomes:
  ...
  Please see the [Usage Examples](#usage-examples) for more in depth examples of drawing content on a page in the new versions of `pdf`. You may also find the [Complete Examples](#complete-examples) to be a useful reference.

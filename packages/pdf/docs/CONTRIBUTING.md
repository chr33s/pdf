...
`pdf` is a JavaScript library designed to create and modify PDF files in any JavaScript environment. Contributions are welcomed and appreciated! Please read through this document to learn how to setup and run the project on your machine. If you have any questions or run into trouble, please [create a discussion](https://github.com/Hopding/pdf/discussions).
...
We try to avoid adding new dependencies to `pdf` as they tend to have a high maintenance cost. However, new dependencies are sometimes necessary. If you're thinking about creating a PR that adds a new dependency, you should write a [proposal](https://github.com/Hopding/pdf/issues/new?assignees=&labels=proposal%2Cneeds-triage&template=proposal.yml) first.
...
You can develop `pdf` on Windows, Mac, or Linux machines. While most of the original code was developed on Macs, care has been taken to ensure that all scripts and commands needed for development are platform independent. (If you find anything that doesn't work on your machine/platform, please [create an issue](https://github.com/Hopding/pdf/issues/new) or submit a PR!)

In order to work on `pdf`, please ensure you have installed the following:
...
```
git clone https://github.com/Hopding/pdf.git
cd pdf
```
...
We use the [Vitest](https://vitest.dev/) framework to write unit tests for `pdf`. All unit tests are kept in the [`test`](./test) directory.
...
We use two linters to keep `pdf`'s source code clean, tidy, and consistent:
...
`pdf` is written in TypeScript. This means that the project's source code is contained in `.ts` files. All source code must be correctly typed in order for the tests to run, and for the project to compile.

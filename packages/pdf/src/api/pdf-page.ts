...
export default class PDFPage {
   ...
   * For example:
   * ```js
   * import { rgb, cmyk, grayscale } from 'pdf'
   *
   ...
   * ```js
   * import { StandardFonts, rgb } from 'pdf'
   *
   * const helveticaFont = await pdfDoc.embedFont(StandardFonts.Helvetica)
   * const timesRomanFont = await pdfDoc.embedFont(StandardFonts.TimesRoman)
   *
   ...
   * import { degrees } from 'pdf'
   *
   * const jpgUrl = 'https://pdf.js.org/assets/cat-riding-unicorn.jpg'
   ...
   * ```js
   * import { degrees, grayscale, rgb } from 'pdf'
   *
   * page.drawRectangle({
   *   x: 25,
   *   y: 75,

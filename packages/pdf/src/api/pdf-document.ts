...
import PDFForm from "./form/pdf-form.js";
import {
  AttachmentOptions,
  Base64SaveOptions,
  CreateOptions,
  ...
  SaveOptions,
  SetTitleOptions,
} from "./pdf-document-options.js";
import PDFEmbeddedFile from "./pdf-embedded-file.js";
import PDFEmbeddedPage from "./pdf-embedded-page.js";
import PDFFont from "./pdf-font.js";
import PDFImage from "./pdf-image.js";
import PDFJavaScript from "./pdf-java-script.js";
...
export default class PDFDocument {
   ...
   *
   * // pdf=ArrayBuffer
   * const url = 'https://pdf.js.org/assets/with_update_sections.pdf'
   ...
   * be embedded. See [here](https://github.com/Hopding/pdf/tree/master#fontkit-installation)
  ...
  getForm(): PDFForm {
    ...
    if (form.hasXFA()) {
      console.warn("Removing XFA form data as pdf does not support reading or writing XFA");
      form.deleteXFA();
    }
    return form;
  }
  ...
  async embedFont(
    ...
    if (isStandardFont(font)) {
      ...
      throw new TypeError("`font` must be one of `string | Uint8Array | ArrayBuffer`");
    }
   ...
   * const pdfDoc = await PDFDocument.create()
   *
   * const sourcePdfUrl = 'https://pdf.js.org/assets/with_large_page_count.pdf'
  ...
  async save(options: SaveOptions = {}): Promise<Uint8Array> {
    ...
    if (updateFieldAppearances) {
      ...
      if (form) await form.updateFieldAppearances();
    }
  ...
  #updateInfoDict(): void {
    const pdfLib = "pdf (https://github.com/chr33s/pdf)";
    const now = new Date();

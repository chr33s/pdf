...
describe("PDFObjectParser", () => {
  ...
  test("throws an error when given empty input", () => {
  ...
  describe("when parsing true booleans", () => {
    test("handles just the 'true' keyword", () => {
      expectParse("true").toBe(PDFBool.True);
    });

    test("handles whitespace before and after the 'true' keyword", () => {
      expectParse("\0\t\n\f\r true\0\t\n\f\r ").toBe(PDFBool.True);
    });

    test("handles comments before and after the 'true' keyword", () => {
      expectParse("% Lulz wut?\ntrue% Lulz wut?\n").toBe(PDFBool.True);
    });
  });

  describe("when parsing false booleans", () => {
    test("handles just the 'false' keyword", () => {
      expectParse("false").toBe(PDFBool.False);
    });

    test("handles whitespace before and after the 'false' keyword", () => {
      expectParse("\0\t\n\f\r false\0\t\n\f\r ").toBe(PDFBool.False);
    });

    test("handles comments before and after the 'false' keyword", () => {
      expectParse("% Lulz wut?\nfalse% Lulz wut?\n").toBe(PDFBool.False);
    });
  });
  ...
  describe("when parsing numbers", () => {
    test.each([
      ...
      ["-.002", "-0.002"],
      ["0.", "0"],
    ])(`handles %s`, (input, output) => {
      expectParse(input).toBeInstanceOf(PDFNumber);
      expectParseStr(input).toBe(output);
    });

    test("handles whitespace before and after the number", () => {
      expectParse("\0\t\n\f\r -.5\0\t\n\f\r ").toBeInstanceOf(PDFNumber);
      expectParseStr("\0\t\n\f\r -.5\0\t\n\f\r ").toBe("-0.5");
    });

    test("handles comments before and after the number", () => {
      expectParse("% Lulz wut?\n-.5% Lulz wut?\n").toBeInstanceOf(PDFNumber);
      expectParseStr("% Lulz wut?\n-.5% Lulz wut?\n").toBe("-0.5");
    });

    test("stops parsing the number when whitespace is encountered", () => {
    ...
    test("stops parsing the number when a delimiter is encountered", () => {
      expectParseStr("12(3").toBe("12");
      expectParseStr("12)3").toBe("12");
      expectParseStr("12<3").toBe("12");
      expectParseStr("12>3").toBe("12");
    ...
    test("can parse several numbers mashed together", () => {
    ...
    test("caps numbers at Number.MAX_SAFE_INTEGER when capNumbers=true", () => {
  ...
  describe("when parsing literal strings", () => {
    ...
    test("handles whitespace before and after the string", () => {
    ...
    test("does not stop parsing the string when whitespace is encountered", () => {
    ...
    test("does not stop parsing the string when a delimiter is encountered", () => {
      expectParseStr("(foo<bar)").toBe("(foo<bar)");
      expectParseStr("(foo>bar)").toBe("(foo>bar)");
      expectParseStr("(foo[bar)").toBe("(foo[bar)");
      expectParseStr("(foo]bar)").toBe("(foo]bar)");
    ...
    test("handles comments embedded within the string", () => {
    ...
    test("respects escaped parenthesis", () => {
      expectParse("(FOO\\(BAR)").toBeInstanceOf(PDFString);
      expectParseStr("(FOO\\(BAR)").toBe("(FOO\\(BAR)");
    });

    test("respects escaped backslashes", () => {
  ...
  describe("when parsing hex strings", () => {
    ...
    test("does not stop parsing the hex string when whitespace is encountered", () => {
  ...
  describe("when parsing names", () => {
    ...
    test("handles names consisting of a single '/'", () => {
      expectParse("/").toBe(PDFName.of(""));
    });

    test("handles whitespace before and after the name", () => {
      expectParse("\0\t\n\f\r /Foo\0\t\n\f\r ").toBe(PDFName.of("Foo"));
    });

    test("handles comments before and after the name", () => {
      expectParse("% Lulz wut?\n/Foo% Lulz wut?\n").toBe(PDFName.of("Foo"));
    });

    test("stops parsing the name when whitespace is encountered", () => {
    ...
    test("stops parsing the name when a delimiter is encountered", () => {
      expectParse("/Foo(Bar").toBe(PDFName.of("Foo"));
      expectParse("/Foo)Bar").toBe(PDFName.of("Foo"));
      expectParse("/Foo<Bar").toBe(PDFName.of("Foo"));
      expectParse("/Foo>Bar").toBe(PDFName.of("Foo"));
    ...
    test("can parse several names mashed together", () => {
    ...
    test("handles names containing non-ASCII characters", () => {
  ...
  describe("when parsing arrays", () => {
    ...
    test("handles empty arrays with whitespace between braces", () => {
      expectParse("[\0\t\n\f\r ]").toBeInstanceOf(PDFArray);
      expectParseStr("[\0\t\n\f\r ]").toBe("[ ]");
    });

    test("handles arrays of all value types seperated by whitespace and (multiple) comments", () => {
      const input = `% Comment
        \0\t\n\f\r % Comment
        [
        \0\t\n\f\r % Comment
    ...
    test("throws an error when closing delimiter is missing", () => {
      expect(() => parse("[/Foo")).toThrow();
    });

    test("throws an error for mismatches delimiters", () => {
      expect(() => parse("[[]")).toThrow();
    });

    test("throws an error when an invalid element is detected", () => {
      expect(() => parse("[/Foo I_AM_INVALID]")).toThrow();
    });
  });

  describe("when parsing dictionaries", () => {
    test("handles empty dictionaries", () => {
      expectParse("<<>>").toBeInstanceOf(PDFDict);
      expectParseStr("<<>>").toBe("<<\n>>");
    });

    test("handles empty dictionaries with whitespace between brackets", () => {
      expectParse("<<\0\t\n\f\r >>").toBeInstanceOf(PDFDict);
      expectParseStr("<<\0\t\n\f\r >>").toBe("<<\n>>");
    });

    test("handles dictionaries of all value types seperated by whitespace and (multiplecomments", () => {
      const input = `% Comment
      \0\t\n\f\r % Comment
      <<
          % Entry 1
    ...
    test("returns the correct subclass based on the dictionary's 'Type'", () => {
      expectParse("<< >>").toBeInstanceOf(PDFDict);
    ...
    test("throws an error for mismatched delimiters", () => {
      expect(() => parse("<<>")).toThrow();
    });

    test("throws an error when an invalid key is detected", () => {
      expect(() => parse("<</Foo/Bar I_AM_INVALID>>")).toThrow();
    });

    test("throws an error when an invalid value is detected", () => {
      expect(() => parse("<</Foo I_AM_INVALID>>")).toThrow();
    });
  });
  ...
  describe("when parsing streams", () => {
    ...
    test("handles streams with a space, carriage return, and a newline following the 'stream' keyword", () => {
      expectParse("<<>>\r\nstream \r\n Stuff and Things \nendstream");
      expectParseStr("<<>>\r\nstream \r\n Stuff and Things \nendstream").toBe(
        "<<\n/Length 18\n>>\nstream\n Stuff and Things \nendstream",
      );
    ...
    test("handles streams with a carriage return and a newline following the 'stream' keyword", () => {
      expectParse("<<>>\r\nstream\r\n Stuff and Things \nendstream");
      expectParseStr("<<>>\r\nstream\r\n Stuff and Things \nendstream").toBe(
        "<<\n/Length 18\n>>\nstream\n Stuff and Things \nendstream",
      );
    ...
    test("handles streams with only a carriage return following the 'stream' keyword", () => {
    ...
    test("handles streams with a carriage return preceding the 'endstream' keyword", () => {
    ...
    test("handles comments and whitespace preceding the 'stream' keyword", () => {
      expectParse(
    ...
    test("handles binary stream content", () => {
  ...
  describe("when parsing null", () => {
    test("handles just the 'null' keyword", () => {
      expectParse("null").toBe(PDFNull);
    });

    test("handles whitespace before and after the 'null' keyword", () => {
      expectParse("\0\t\n\f\r null\0\t\n\f\r ").toBe(PDFNull);
    });

    test("handles comments before and after the 'null' keyword", () => {
      expectParse("% Lulz wut?\nnull% Lulz wut?\n").toBe(PDFNull);
    });
  });

  describe("when parsing indirect object references", () => {
    test("handles whitespace before and after the ref", () => {
      expectParse("\0\t\n\f\r 1 2 R\0\t\n\f\r ").toBe(PDFRef.of(1, 2));
    });

    test("handles whitespace within the ref", () => {
      expectParse("1\0\t\n\f\r2\0\t\n\f\rR").toBe(PDFRef.of(1, 2));
    });

    test("handles comments before and after the ref", () => {
      expectParse("% Lulz wut?\n1 2 R% Lulz wut?\n").toBe(PDFRef.of(1, 2));
    });

    test("handles comments within the ref", () => {
      expectParse("1% Lulz wut?\r2% Lulz wut?\rR").toBe(PDFRef.of(1, 2));
    });

    test("does not stop parsing the ref when whitespace is encountered", () => {
    ...
    test("stops parsing the ref when a delimiter is encountered", () => {
    ...
    test("can parse several refs mashed together", () => {
    ...
    test("can parse a number, then a ref, then a number", () => {

...
describe("StandardFontEmbedder", () => {
  test("can be constructed with StandardFontEmbedder.for(...)", async () => {
    const embedder = await StandardFontEmbedder.for(FontNames.Helvetica);
    expect(embedder).toBeInstanceOf(StandardFontEmbedder);
  });

  test("exposes the font's name", async () => {
    const embedder = await StandardFontEmbedder.for(FontNames.HelveticaOblique);
    expect(embedder.fontName).toBe("Helvetica-Oblique");
  });

  test("can use a custom font name", async () => {
    const customName = "Roboto 2";
    const embedder = await StandardFontEmbedder.for(FontNames.HelveticaOblique, customName);
    expect(embedder.customName).toBe(customName);
  });

  test("can embed standard font dictionaries into PDFContexts without a predefined ref", async () => {
    const context = PDFContext.create();
    const embedder = await StandardFontEmbedder.for(FontNames.Courier);
  ...
  test("can embed standard font dictionaries into PDFContexts with a predefined ref", async () => {
    const context = PDFContext.create();
  ...
  test("can encode text strings into PDFHexString objects", async () => {
    ...
    const embedder = await StandardFontEmbedder.for(FontNames.TimesRoman);
  ...
  test("can measure the width of text strings at the given font size", async () => {
    const text = "Stuff and thingz!";
    const embedder = await StandardFontEmbedder.for(FontNames.HelveticaBold);

...
describe("FlateStream", () => {
  test.each(FILES)("can decode flate encoded data (%s)", async (file) => {
    const encoded = new Uint8Array(
      await readFile(new URL(`./data/flate/${file}.encoded`, import.meta.url)),
    );
    const decoded = new Uint8Array(
      await readFile(new URL(`./data/flate/${file}.decoded`, import.meta.url)),
    );

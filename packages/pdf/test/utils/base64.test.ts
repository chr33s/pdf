...
describe("decodeFromBase64DataUri", () => {
  ...
  test("can decode partial data URIs (1)", () => {
    const uri = `data:application/pdf;,${pdfBase64String}`;
    expect(arraysAreEqual(decodeFromBase64DataUri(uri), pdfBytes)).toBe(true);
  });

  test("can decode partial data URIs (2)", () => {
    const uri = `data:;,${pdfBase64String}`;
    expect(arraysAreEqual(decodeFromBase64DataUri(uri), pdfBytes)).toBe(true);
  });

  test("can decode partial data URIs (3)", () => {
    const uri = `data:application/pdf;,${pdfBase64String}`;
    expect(arraysAreEqual(decodeFromBase64DataUri(uri), pdfBytes)).toBe(true);
  });

  test("can decode partial data URIs (4)", () => {
    const uri = `:;,${pdfBase64String}`;
    expect(arraysAreEqual(decodeFromBase64DataUri(uri), pdfBytes)).toBe(true);
  });

  test("can decode partial data URIs (5)", () => {
    const uri = `;,${pdfBase64String}`;
    expect(arraysAreEqual(decodeFromBase64DataUri(uri), pdfBytes)).toBe(true);
  });

  test("can decode partial data URIs (6)", () => {
    const uri = `,${pdfBase64String}`;
    expect(arraysAreEqual(decodeFromBase64DataUri(uri), pdfBytes)).toBe(true);
  });

  test("can throws an error when it fails to parse the URI", () => {
    const uri = {} as any;

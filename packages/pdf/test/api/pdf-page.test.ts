...
describe("PDFDocument", () => {
  describe("getSize() method", () => {
    test("returns the width and height of the the page's MediaBox", async () => {
  ...
  describe("setSize() method", () => {
    test("sets the width and height of only the the page's MediaBox when the other boxes are not defined", async () => {
      const pdfDoc = await PDFDocument.create();
      const page = pdfDoc.addPage();
    ...
    test("sets the width and height of the the page's CropBox, BleedBox, TrimBox, and ArtBox when they equal the MediaBox", async () => {
      const pdfDoc = await PDFDocument.create();
      const page = pdfDoc.addPage();
    ...
    test("does not set the width and height of the the page's CropBox, BleedBox, TrimBox, or ArtBox when they do not equal the MediaBox", async () => {
      const pdfDoc = await PDFDocument.create();
      const page = pdfDoc.addPage();
  ...
  // https://github.com/Hopding/pdf/issues/1075
  test("drawImage() does not reuse existing XObject keys", async () => {
  ...
  // https://github.com/Hopding/pdf/issues/1075
  test("setFont() does not reuse existing Font keys", async () => {
    const pdfDoc1 = await PDFDocument.create();

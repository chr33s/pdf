...
describe("svgPathToOperators", () => {
  ...
  test("can map rectangle with lines paths to PDF operators", () => {
    ...
    expect(operators.toString()).toBe("50 50 m,50 100 l,100 100 l,100 0 l,50 0 l,h");
  });

  test("can map bezier curve paths to PDF operators", () => {
    const operators = svgPathToOperators("M100,200 C100,100 250,100 250,200 S400,300 400,200");
  ...
  test("can map relative bezier curve paths to PDF operators", () => {
    const operators = svgPathToOperators("M100,200 c0,-100 150,-100 150,0 s150,100 150,0");
    expect(operators.length).toBe(3);
    expect(operators.toString()).toBe(
      "100 200 m,100 100 250 100 250 200 c,250 300 400 300 400 200 c",
    );
  ...
  test("can map quadratic curve paths to PDF operators", () => {
    ...
    expect(operators.length).toBe(3);
    expect(operators.toString()).toBe("200 300 m,400 50 600 300 v,800 550 1000 300 v");
  });
  ...
  test("can map arc paths to PDF operators", () => {
    const operators = svgPathToOperators("M300,200 h-150 a150,150 0 1,0 150,-150 z");
    expect(operators.length).toBe(6);
    expect(operators.toString()).toBe(
  ...
  test("can map relative arc paths to PDF operators", () => {
    const operators = svgPathToOperators("M300,200 h-150 A150,150 0 1,0 300,50 z");
    expect(operators.length).toBe(6);
    expect(operators.toString()).toBe(
      "300 200 m,150 200 l,150 282.8427124746191 217.15728752538098 350 300 350 c,382.84271247461896 350 450 282.842712474619 450 200.00000000000003 c,450 117.15728752538101 382.84271247461896 50.00000000000003 300 50.00000000000002 c,h",
    );
  ...
  test("can map cubic bezier paths to PDF operators", () => {
    const operators = svgPathToOperators("m 100 100 S 200,100 200,200 200,200 100,200 T 0,250");
    expect(operators.length).toBe(4);
    expect(operators.toString()).toBe(
  ...
  test("can map dashed line paths to PDF operators", () => {
    const operators = svgPathToOperators("M 0,25 5,25 M 10,25 15,25 M 20,25 25,25");
    expect(operators.length).toBe(6);
    expect(operators.toString()).toBe("0 25 m,5 25 l,10 25 m,15 25 l,20 25 m,25 25 l");
  });
  ...
  test("can map paths that reset cubic bezier control points to PDF operators", () => {
    const operators = svgPathToOperators("M100 100 S200,100 200,200 M95 105 s100,0 100,100");
    expect(operators.length).toBe(4);
    expect(operators.toString()).toBe(
  ...
  test("can map paths that reset quadratic bezier control points to PDF operators", () => {
    const operators = svgPathToOperators("M100 100 T100,200 200,200 M90,120 t0,100 100,0");
    expect(operators.length).toBe(6);
    expect(operators.toString()).toBe(
  ...
  test("correctly updates control points for T command", () => {
    // See https://github.com/Hopding/pdf/issues/1443
    const operators = svgPathToOperators("M 10,25 Q 30,0 50,25 Q 70,50 90,25 T 130,25 T 170,25");

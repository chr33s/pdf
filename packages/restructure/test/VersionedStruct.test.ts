...
import { VersionedStruct, String as StringT, Pointer, uint8, DecodeStream, EncodeStream } from '../src';
...
describe('VersionedStruct', () => {
  ...
  describe('decode', () => {
    it('should get version from number type', () => {
      ...
      expect(baseStruct.decode(stream0)).to.deep.equal({ version: 0, name: 'devon', age: 21 });

      const stream1 = new DecodeStream(Buffer.from('\x01\x0adevon 👍\x15\x00', 'utf8'));
      expect(baseStruct.decode(stream1)).to.deep.equal({ version: 1, name: 'devon 👍', age: 21, gender: 0 });
    });
    ...
    it('should support common header block', () => {
      ...
      expect(struct.decode(stream0)).to.deep.equal({ version: 0, age: 21, alive: 1, name: 'devon' });

      const stream1 = new DecodeStream(Buffer.from('\x01\x15\x01\x0adevon 👍\x00', 'utf8'));
      expect(struct.decode(stream1)).to.deep.equal({ version: 1, age: 21, alive: 1, name: 'devon 👍', gender: 0 });
    });
    ...
    it('should support parent version key', () => {
      ...
      expect(struct.decode(stream0, { version: 0 })).to.deep.equal({ version: 0, name: 'devon', age: 21 });

      const stream1 = new DecodeStream(Buffer.from('\x0adevon 👍\x15\x00', 'utf8'));
      expect(struct.decode(stream1, { version: 1 })).to.deep.equal({ version: 1, name: 'devon 👍', age: 21, gender: 0 });
    });
    ...
    it('should support sub versioned structs', () => {
      ...
      expect(struct.decode(stream1, { version: 0 })).to.deep.equal({ version: 0, name: 'pasta' });

      const stream2 = new DecodeStream(Buffer.from('\x01\x01\x09ice cream\x01', 'binary'));
      expect(struct.decode(stream2, { version: 0 })).to.deep.equal({ version: 1, name: 'ice cream', isDesert: 1 });
    });
    ...
    it('should support process hook', () => {
      ...
      expect(struct.decode(stream)).to.deep.equal({ version: 0, name: 'devon', age: 21, processed: true });
    });
  });
  ...
  describe('size', () => {
    ...
    it('should throw for unknown version', () => {
      expect(() => baseStruct.size({ version: 5, name: 'devon', age: 21 })).to.throw(/unknown version/i);
    });
    ...
    it('should support common header block', () => {
      ...
      expect(struct.size({ version: 1, age: 21, alive: 1, name: 'devon 👍', gender: 0 })).to.equal(15);
    });
  ...
  describe('encode', () => {
    it('should encode objects to buffers', (done) => {
      ...
      baseStruct.encode(stream, { version: 1, name: 'devon 👍', age: 21, gender: 0 });
      stream.end();
    });

    it('should throw for unknown version', () => {
      const stream = new EncodeStream();
      expect(() => baseStruct.encode(stream, { version: 5, name: 'devon', age: 21 })).to.throw(/unknown version/i);
    });
    ...
    it('should support common header block', (done) => {
      ...
      const stream = new EncodeStream();
      expectStream(stream, done, (buf) => {
        expect(buf).to.deep.equal(Buffer.from('\x00\x15\x01\x05devon\x01\x15\x01\x0adevon 👍\x00', 'utf8'));
      });

      struct.encode(stream, { version: 0, age: 21, alive: 1, name: 'devon' });
      struct.encode(stream, { version: 1, age: 21, alive: 1, name: 'devon 👍', gender: 0 });
      stream.end();
    });
    ...
    it('should encode pointer data after structure', (done) => {
      ...
      struct.encode(stream, { version: 1, name: 'devon', age: 21, ptr: 'hello' });
      stream.end();
    });

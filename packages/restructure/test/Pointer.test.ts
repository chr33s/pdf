...
import { Pointer, VoidPointer, uint8, DecodeStream, EncodeStream, Struct } from '../src';
...
describe('Pointer', () => {
  ...
  describe('encode', () => {
    ...
    it('should handle global offsets', (done) => {
      ...
      expect(ctx.parent.parent.parent.pointers).to.deep.equal([{ type: uint8, val: 10, parent: ctx }]);
      stream.end();
    });

...
import { degrees, LineCapStyle, ParseSpeeds, PDFDocument, rgb, StandardFonts } from "@chr33s/pdf";
import { assets } from "./assets.js";

test("Test 2: Load PDF with update sections", async () => {
  const pdfDoc = await PDFDocument.load(await assets.pdfs.withUpdateSectionsBase64Uri(), {
    parseSpeed: ParseSpeeds.Fastest,
    updateMetadata: false,
  });

  await pdfDoc.attach(await assets.pdfs.normalBase64(), "tax_form.pdf", {
  ...
  const catRidingUnicornImage = await pdfDoc.embedJpg(await assets.images.jpg.catRidingUnicorn());
  const cmykImage = await pdfDoc.embedJpg(await assets.images.jpg.cmykColorspace());
  ...
  const docWithAnnots = await PDFDocument.load(await assets.pdfs.withAnnots());

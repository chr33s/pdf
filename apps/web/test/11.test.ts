...
const thirdPage = async (pdfDoc: PDFDocument, assets: Assets) => {
  ...
  const pngImage = await pdfDoc.embedPng(await assets.images.png.selfDrive());
  ...
  const [embeddedPage] = await pdfDoc.embedPdf(await assets.pdfs.simplePdf2Example(), [0]);
...
test("Test 11: SVG paths and blend modes", async () => {

...
test("Test 5: PDF with missing endstream EOL", async () => {
  ...
  await pdfDoc.attach(await assets.pdfs.usConstitution(), "us-constitution.pdf", {
  ...
  await pdfDoc.attach(await assets.images.jpg.catRidingUnicornBase64(), "cat-riding-unicorn.jpg", {
    mimeType: "image/jpeg",
  ...
  const nunitoLigaFont = await pdfDoc.embedFont(await assets.fonts.ttf.nunito(), {
    subset: true,
    features: { liga: true },
  });
  const nunitoNoLigaFont = await pdfDoc.embedFont(await assets.fonts.ttf.nunito(), {
    subset: true,
    features: { liga: false },
  });
  const smallMarioImage = await pdfDoc.embedPng(await assets.images.png.smallMario());
  const smallMarioDims = smallMarioImage.scale(0.15);

  const page1 = pdfDoc.getPage(0);

  const text =
    "This is an image of Mario running. This image and text was drawn on top of an existing PDF using pdf!";
  const fontSize = 24;
  ...
  const pdfDocWithCropBox = await PDFDocument.load(await assets.pdfs.withCropbox());

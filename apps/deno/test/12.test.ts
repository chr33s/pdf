...
const pngSuiteUrl = (file: string) =>
  new URL(`../../../packages/pdf/assets/images/pngsuite/${file}`, import.meta.url);

Deno.test("Test 12: PNG suite embedding", async () => {
  ...
  for (const path of pngSuite) {
    const png = await Deno.readFile(pngSuiteUrl(path));

...
const loadA = async (assets: Assets) => {
  const pdfDoc = await PDFDocument.load(await assets.pdfs.withCombedFields());
...
const loadB = async (assets: Assets) => {
  const pdfDoc = await PDFDocument.load(await assets.pdfs.dodCharacter());
  const form = pdfDoc.getForm();

  const marioImage = await pdfDoc.embedPng(await assets.images.png.smallMario());
...
const loadC = async (assets: Assets) => {
  const pdfDoc = await PDFDocument.load(await assets.pdfs.withXfaFields());
  const form = pdfDoc.getForm();
...
const loadD = async (assets: Assets) => {
  ...
  const ubuntuFont = await pdfDoc.embedFont(await assets.fonts.ttf.ubuntu());
...
// Based on https://github.com/Hopding/pdf#create-form
...
test("Test 17: Form flattening", async () => {
  const pdfDoc = await PDFDocument.load(await assets.pdfs.formToFlatten());

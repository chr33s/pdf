...
test("Test 12: PNG suite embedding", async () => {
  ...
  const etweImg = await pdfDoc.embedPng(await assets.images.png.etwe());
  ...
  const selfDriveImg = await pdfDoc.embedPng(await assets.images.png.selfDrive());
  ...
  for (const path of pngSuite) {
    const png = await assets.images.png.pngSuite(path);

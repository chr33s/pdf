...
test("Test 1: Load and modify existing PDF", async () => {
  const pdfDoc = await PDFDocument.load(await assets.pdfs.linearizedWithObjectStreams(), {
  ...
  const sourcePdfDoc = await PDFDocument.load(await assets.pdfs.withLargePageCount());
  ...
  // pdf does not currently support reading metadata streams.
